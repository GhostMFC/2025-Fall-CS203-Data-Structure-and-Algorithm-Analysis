...
private:
  ...
  AVLNode* Zig(AVLNode* x){//left rotate->heights right-left==2
    AVLNode *y=x->rch,*T2=y->lch; y->lch=x,x->rch=T2;
    update(x); update(y); return y;//new root
  }
  AVLNode* Zag(AVLNode* y){//right rotate
    AVLNode *x=y->lch,*T2=x->rch; x->rch=y,y->lch=T2;
...
const int N=300010,M=50010;

int n,m,a[N];
struct Q{int l,r,k,num,ans;}q[M];
...
signed main(){
  ...
  for(int i=1;i<=n;++i)
    cin>>a[i];
  for(int i=1;i<=m;++i)
    cin>>q[i].l>>q[i].r>>q[i].k,q[i].num=i;
  ...
  int l=1,r=0;
  for(int i=1;i<=m;++i){
    while(r<q[i].r) tr.insert(a[++r]);
    while(l<q[i].l) tr.remove(a[l++]);
    q[i].ans=tr.getKth(q[i].k);
  }
  sort(q+1,q+1+m,[&](Q A,Q B){
    return A.num<B.num;
  });
